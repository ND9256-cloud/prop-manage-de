...
- **Derived fields out of scope (deferred to 4.3c)**: `unit_ref` (and any composite/derived field) is marked `derived: true` → `derived_pending`, excluded from the grounding aggregate, no grade assigned. Structured fields (tenant/landlord_identity) are `non_scalar` and likewise excluded.

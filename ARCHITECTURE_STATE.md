...
## Task 4.3c-b-ii-B — extractor now EMITS `table_cell` evidence for `unit_ref` (schema-driven prose) + Lena re-extracted (2026-06-07)

- **The extractor now EMITS `table_cell` evidence for `unit_ref` when the floor comes from a table — schema-driven, no wrapper edit.** ii-A taught the production envelope to CARRY/shape-VALIDATE/RENDER `table_cell`; ii-B is the EMISSION + eval proof. The change is PROSE-ONLY in `schemas/mietvertrag/schema.yaml` `prompt_fragment_template` (the `unit_ref` section): when-to-use guidance for `direct_quote` (default, clean phrase) vs `table_cell` (floor from a table cell), the exact nested shape (`column_anchor`/`row_anchor` are OBJECTS with a `quote` field — NOT bare strings), `cell_value_raw` MUST be the RAW token (anti-laundering: never the normalized `"1.OG"` unless `"1.OG"` literally grounds), `row_anchor` ONLY for multi-row disambiguation (OMIT for a single-unit description table; never invent one from a tenant name/address outside the row), no floor from an ambiguous column (`Nr./Zimmer/Anzahl/Pos.`), and `absence_state: ambiguous` when the row cannot be identified. Regenerated `schemas/mietvertrag/generated/prompt_fragment.ts` only (field_specs / envelope_validator / field_labels byte-unchanged — prose-only edit); `gen:schemas:check` green.
- **Trigger path inherits via the shared generated fragment (no duplicated wrapper edit).** Both production `supabase/functions/process-document/index.ts` and the eval `scripts/eval/extractor.ts` import `PROMPT_FRAGMENT` from the generated file, so the pg_cron `process-document` extractor and the eval extractor pick up the guidance identically. The system-prompt WRAPPER (which already describes both evidence shapes, added in ii-A) was NOT touched, so `extractor-drift.test.ts` stays green.
- **Lena re-extracted (one live Sonnet call) → `unit_ref` is now a grounded `table_cell`, scored 0 → 2 (HONEST, not forced to 3).** `eval/candidates/lena-iiB` carries `unit_ref` evidence `{ evidence_type:"table_cell", page:2, table_cell:{ column_anchor:{quote:"Geschoss"}, cell_value_raw:"1", derivation_rule:"geschoss_numeric_to_og" } }` — RAW token (not `"1.OG"`), correct floor column, `row_anchor` correctly OMITTED (her floor row `"100,00 m² 1 3,5 1 1 0 1 – Mitte 0"` carries no tenant/unit name; inventing one from Lena's page-1 name is what the guidance forbids). The scorer caps at **2**, not 3, because page 2's OCR linearization scatters stray standalone `"1"` tokens (the key-count row `"3 4 1 1 – Garage"`, the footnote `"1)"`) at/after the `Geschoss` header → the deterministic candidate-data-row counter (4.3c-b-1b-i) sees a MULTI-row block, which REQUIRES a `row_anchor` for grade 3. This is the brief's explicitly-accepted outcome: the evidence is HONEST (0 → 2), and forcing grade 3 would require an invented row anchor (overclaiming). Roll-up `grounding_grade_mean` for the everding fixture rises 2.40 → **2.80** (5 graded fields, `unit_ref` 0→2); `grade3_rate` unchanged at 0.800.
- **Three held-out shapes verified (anti-overfit, §3) by EXTENDING `table-cell-grounding.test.ts`** (now 44 assertions, +5) rather than adding fixture dirs — the brief's sanctioned "(or extend table-cell-grounding coverage)" path, lower blast-radius than new fixtures and already CI-wired. Each uses DISTINCT headers/columns/names from Lena and from every prior case: (1) multi-row rent-roll (`Mieter|Etage|Kaltmiete`, two tenants on Etage 2) → `row_anchor` PRESENT and disambiguating → 3, same table with NO anchor → ≤2; (2) single-row description table (`Wohnfläche|Etage|Zimmer|Bad`, `EG` passthrough, no tenant in row) → `row_anchor` OMITTED → 3; (3) ambiguous numeric column (`"1"` under `Pos.` and under `Anzahl`, even when the model lies `canonical:"floor"`) → NOT grade 3 (0).
- **Reconciliation (per the illustrative-brief rule)**: (a) the brief's `extract` command omits `--fixture-cap`, which `run.ts` requires — ran with `--fixture-cap 1`; (b) the model first emitted `column_anchor` as a bare string `"Geschoss"` (scorer expects `{quote}`), so the prose was tightened to spell out the nested object shape, then re-extracted; (c) §3 held-outs implemented as extended scorer-test coverage (the OR branch) instead of new fixture dirs.
- **Tests**: `table-cell-grounding.test.ts` 44 assertions (incl. 5 new held-out); `tsc --noEmit` clean; `gen:schemas:check` green; ALL ten DB-free eval tests + both evidence tests unchanged-green; `everding-end-to-end` integration gate (25 assertions, real DB chain emitter→applier→resolver = €650) green with the self-loading env. No production emitter/claim/DB change; no `warehouse.evidence` persistence; UI provenance for `table_cell` remains deferred to ii-C.
- Out of scope (unchanged, → ii-C): NO emitter/claim/DB change; NO `warehouse.evidence` persistence; NO UI provenance for `table_cell`; NO `envelope_validator` change (still array-only; standalone `validateEvidence` unchanged); NO composite/address, money `table_cell`, or bbox; `direct_quote` behavior, scalar/identity/derived grade logic, `normalized_match`, and applier logic untouched.

## Task 4.3c-b-1b-i-2 — table_cell ambiguity counts only rows under the licensed floor column (scorer-only) (2026-06-07)

- **The multi-row ambiguity test now counts ONLY column-aligned candidate rows, not bare token occurrences anywhere on the page.** 1b-i computed `multiRow` from `valueLines.filter(vi => vi >= headerMin)` — EVERY value line carrying the raw token at/after the first floor-header line. On Lena's page 2 that over-counted: the real floor row (`"100,00 m² 1 3,5 1 1 0 1 – Mitte 0"` under `Geschoss`) AND an unrelated garage row (`"3 4 1 1 – Garage"`, >10 lines later, no floor header nearby) both carry a standalone `"1"`, so the block read as MULTI-row → `row_anchor` required → her correctly-anchor-less single floor capped at 2 instead of 3.
- **Fix (`scripts/eval/metrics.ts` `gradeTableCellOnPage`)**: a candidate row counts ONLY when its raw-token line ALSO has the licensed (floor) column header within the SAME locality window used for grade-3 column grounding — a header in the previous ≤10 lines. Replaced the `vi >= headerMin` filter with `candidateRows = valueLines.filter(vi => col.indices.some(ci => ci <= vi && vi - ci <= 10))`, i.e. the existing per-value-line `columnOk` rule. `multiRow = candidateRows.length >= 2`. The garage row (header at line 1, garage `"1"` >11 lines below) is no longer column-aligned → single-row → grade 3 without a row anchor. Genuine multi-row protection is unchanged: two real floor rows under ONE `Geschoss` header are both inside its locality → 2 candidate rows → MULTI-row → `row_anchor` still REQUIRED for 3.
- **Lena re-score (`eval/candidates/lena-iiB`, no re-extraction)**: `unit_ref` moves **2 → 3** — the garage row no longer counts as a competing floor row, so her single floor row reaches 3 with the (correctly omitted) row anchor. Fixture roll-up `grounding_grade_mean` rises 2.80 → **3.00** (5 graded fields all grade 3); `grade3_rate` 0.800 → **1.000**. The gold-grounding invariant stays green.
- **Tests** (`src/tests/eval/table-cell-grounding.test.ts`, now 49 assertions, +5): Lena-shape floor row + unrelated garage row (>10 lines from the header, non-floor `Stockwerk`/`Anzahl` columns) → 3 without a row anchor (was 2 under 1b-i); genuine two-floor-row rent-roll (`Mieter|Geschoss|Miete`, two tenants on Geschoss 2) → ≤2 without anchor, 3 with a disambiguating anchor. ALL prior 44 assertions unchanged-green (single-row, wrong-column, ambiguous-header, anti-laundering, duplicate-rows→≤2, multi-row±anchor, wrapped-value, held-out shapes) — no prior fixture had an off-column token, so none flipped.
- Out of scope (unchanged): no production / `envelope_validator` change; no re-extraction; no Sonnet; locality window sizes, the column-licensing rule, anti-laundering, and other grade paths untouched; genuine multi-row protection NOT loosened. Touches only `scripts/eval` + tests (not a trigger path).
